...
{	
    ...
    for(int i = 0 ; i < plane.pores.size() ; i++){
        ...
        while (p1!=NULL){
            ...
            double xleftup = myinterpolate(p1->moLeft.mNode1.y,p1->mdEndPosition,p1->moLeft.mNode2.y,p1->moLeft.mNode1.x,p1->moLeft.mNode2.x);
            double xleftdown = myinterpolate(p1->moLeft.mNode1.y,p1->mdStartPosition,p1->moLeft.mNode2.y,p1->moLeft.mNode1.x,p1->moLeft.mNode2.x);
            double xrightup = myinterpolate(p1->moRight.mNode1.y,p1->mdEndPosition,p1->moRight.mNode2.y,p1->moRight.mNode1.x,p1->moRight.mNode2.x);
            double xrightdown = myinterpolate(p1->moRight.mNode1.y,p1->mdStartPosition,p1->moRight.mNode2.y,p1->moRight.mNode1.x,p1->moRight.mNode2.x);
                
            int xEle = int((xrightdown - xleftdown)/windowssizex + 0.5);
            int yEle = int((p1->mdEndPosition - p1->mdStartPosition)/windowssizex + 0.5);
            mothervertex(tempMotherElement, xleftup, xleftdown, xrightup, xrightdown, p1->mdStartPosition,p1->mdEndPosition, p1->moLeft.mNode1.z, xEle,yEle);
